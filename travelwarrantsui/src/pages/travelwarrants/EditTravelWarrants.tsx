...
import { getTravelWarrantsById } from "../../api/api";
...
const EditTravelWarrants = () => {
  ...
  const { editTour } = useContext(TravelWarrantsContext);
  ...
  useEffect(() => {
    const fetchTourData = async () => {
      try {
        const tourData = await getTravelWarrantsById(Number(id));
        setAddTravelWarrant(tourData);
      } catch (error) {
  ...
  const handleSubmit = async (
    ...
    try {
      await editTour(Number(id), addTravelWarrant);
  ...
  return (
    <div>
      <div>
        ...
        <h1 style={{ padding: "0px" }}>Izmijeni putni nalog</h1>
      </div>
...
export default EditTravelWarrants;
